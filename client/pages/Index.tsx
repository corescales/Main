...
import { Check, Clock3, Gauge, Layers3, Puzzle, Sparkles, SplitSquareHorizontal, TrendingUp } from "lucide-react";
...
export default function Index() {
  ...
  return (
    <div className="relative">
      ...
      <Testimonials />
      <CTA />
      <span className="hidden" aria-hidden>{exampleFromServer}</span>
    </div>
  );
}
        ...
        <div className="mx-auto max-w-3xl text-center">
          ...
          <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Button asChild size="lg" className="px-6">
              <a href="#contact-cta">Book a strategy call</a>
            </Button>
            <a
              href="#features"
...
function Features() {
  const items = [
    { icon: Layers3, title: "Modular blocks", desc: "Compose reusable, brand-safe sections to build pages in minutes." },
    { icon: SplitSquareHorizontal, title: "A/B testing", desc: "Experiment with variants, roll out winners, and keep momentum." },
    { icon: Puzzle, title: "Integrations", desc: "Connect analytics, forms, and your CMS. No fragile glue code." },
    { icon: Gauge, title: "Performance", desc: "Lean, accessible pages optimized for Core Web Vitals and SEO." },
    { icon: Clock3, title: "Faster workflows", desc: "Cut cycles with a visual editor that reflects your brand system." },
    { icon: TrendingUp, title: "Measurable growth", desc: "Dashboards that tie experiments to pipeline and revenue." },
  ];

  return (
    <section id="features" className="container mx-auto px-4 py-24">
      <div className="mx-auto max-w-2xl text-center">
        <h2 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Everything you need to scale</h2>
        <p className="mt-3 text-foreground/70">Ship bold ideas without waiting on sprint cycles.</p>
      </div>
  ...
  return (
    <div className="grid grid-cols-12 gap-4">
      <div className="col-span-3 hidden rounded-lg border border-border/60 bg-background p-3 sm:block">
        <p className="text-xs font-semibold uppercase tracking-wide text-foreground/60">Blocks</p>
        <div className="mt-3 grid gap-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-9 rounded-md border border-dashed border-border/70"></div>
          ))}
        </div>
      </div>
    ...
    <section className="container mx-auto px-4 py-10">
      <div className="mx-auto max-w-4xl rounded-xl border border-border/60 bg-card p-6 shadow-sm">
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-3">
          {["Drag & drop blocks", "Instant responsive", "Publish with guardrails"].map((t) => (
            <div key={t} className="flex items-start gap-3 rounded-lg border border-border/60 bg-background p-3 text-sm">
  ...
  return (
    <section className="container mx-auto px-4 py-24">
      <div className="grid items-center gap-10 lg:grid-cols-2">
        <div>
          <h2 className="text-3xl font-extrabold tracking-tight sm:text-4xl">Results that compound</h2>
    ...
    <section id="contact-cta" className="relative overflow-hidden py-16">
      ...
      <div className="container mx-auto px-4">
        <div className="mx-auto max-w-3xl rounded-2xl border border-border/60 bg-gradient-to-tr from-primary/15 via-emerald-400/10 to-transparent p-8 text-center shadow-xl">
          <h3 className="text-2xl font-extrabold tracking-tight sm:text-3xl">Ready to scale your brand?</h3>
          <p className="mt-2 text-foreground/70">Book a strategy call and we’ll design a plan tailored to your goals.</p>
          <div className="mt-6 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Button size="lg" className="px-6">Book a strategy call</Button>
            <a href="/pricing" className="text-sm font-semibold text-primary hover:opacity-90">See pricing</a>
          </div>
        </div>
      </div>
    </section>
